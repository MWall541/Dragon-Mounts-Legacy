...
//        if (dragon.isInWater()) animBase *= 0.4;
        ...
        cycleOfs *= MathX.terpLinear(0.5f, 1, flutter);
        cycleOfs *= MathX.terpLinear(1, 0.5f, ground);
    ...
    public void tick()
    {
        ...
        if (initTrails)
        {
            yTrail.fill((float) dragon.getPosY());
        ...
        float speedEnt = (float) (dragon.getMotion().x * dragon.getMotion().x + dragon.getMotion().z * dragon.getMotion().z);
        ...
        yTrail.update((float) dragon.getPosY());
    ...
    protected void animHeadAndNeck(DragonModel model)
    {
        ...
        for (int i = 0; i < model.neckProxy.length; i++)
        {
            ...
            baseRotX *= MathX.terpLinear(0.2f, 1, flutter);
            baseRotX *= MathX.terpLinear(1, 0.2f, sit);
            ...
            model.neck.rotateAngleX *= MathX.terpSmoothStep(1, 0.5f, walk);
    ...
    protected void animTail(DragonModel model)
    {
        ...
        for (int i = 0; i < model.tailProxy.length; i++)
        {
            ...
            rotXAir -= MathX.sin(i * 0.45f + animBase) * 0.04f * MathX.terpLinear(0.3f, 1, flutter);

            // interpolate between sitting and standing
            model.tail.rotateAngleX = MathX.terpLinear(rotXStand, rotXSit, sit);
            model.tail.rotateAngleY = MathX.terpLinear(rotYStand, rotYSit, sit);

            // interpolate between flying and grounded
            model.tail.rotateAngleX = MathX.terpLinear(rotXAir, model.tail.rotateAngleX, ground);
            model.tail.rotateAngleY = MathX.terpLinear(rotYAir, model.tail.rotateAngleY, ground);

            // body movement
            float angleLimit = 160 * vertMulti;
            float yawOfs = MathX.clamp(yawTrail.get(partialTicks, 0, i + 1) * 2, -angleLimit, angleLimit);
            float pitchOfs = MathX.clamp(pitchTrail.get(partialTicks, 0, i + 1) * 2, -angleLimit, angleLimit);
    ...
    protected void animLegs(DragonModel model)
    {
        ...
        for (int i = 0; i < model.thighProxy.length; i++)
        {
            ...
            // interpolate between sitting and standing
            yGround = MathX.terpSmoothStep(yGroundStand[i % 2], yGroundSit[i % 2], sit);
            ...
            toe.rotateAngleX = MathX.terpSmoothStep(xAir[3], xGround[3], ground);
...
//        }
        ...
        if (shift) MathX.terpCatmullRomSpline(xn, result, a2, a3, a1, a2);
        else MathX.terpCatmullRomSpline(xn, result, a1, a2, a3, a1);
    }
    ...
    public float getModelPitch(float pt)
    {
        ...
        float pitchHover = 60;
        return MathX.terpSmoothStep(pitchHover, pitchMoving, speed);
    }
